...
function createQuerySql(size: number = 10, current: number = 1, query: Partial<Mission> = {}) {
  ...
  const selectSql = `SELECT ${tbm}.*, JSON_OBJECT(${joinObjStr}) AS launchVehicle FROM ${tbm} JOIN ${tbv} ON ${tbm}.vehicleId = ${tbv}.id WHERE ${queryStr} ORDER BY launchTime DESC LIMIT ${size} OFFSET ${offset};`
