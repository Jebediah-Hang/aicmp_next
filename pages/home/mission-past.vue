<template>
  <div v-show="!isShowDetail">
    ...
    <div class="mission-list">
      <HomeMissionPast
        v-for="mission in pastMissionList"
        :key="mission.id"
        :mission="mission"
        @detail="handleDetail"
      />
...
const pastMissionList = ref<Array<Mission>>([]);
function getPastMission() {
  ...
  $request<ApiPageList<Mission>>({
    url: '/api/service/mission/past',
    method: 'GET',
    params: queryParams
  }).then(res => {
    const { code, data } = res;
    if (code === 200 && data) {
      pastMissionList.value = data.list;
